...
(deftest extended-by-test
  (let [element0 '(3 "foo")
    ...
    (is (not (let-propagated [a '(3 ("foo" false)) b '(3 ("foo"))]
                             (extended-by? a b))))
    (is (let-propagated [a element0] (extended-by? 3 a)))
...
(deftest template-matches-test
  ...
  (is (= (let-propagated [x '(1)] (template-matches x {:a :b} x)) [{:a :b}]))
  (is (= (let-propagated [x '(1 2)] (template-matches 1 {:a :b} x)) [{:a :b}]))
  (is (= (let-propagated [x '(1) y '(1 2)] (template-matches x {:a :b} y))
         [{:a :b}]))
  (is (= (let-propagated [x '(1 2)] (template-matches x {:a :b} x)) [{:a :b}]))
