(ns cosheet2.server.hierarchy-test
  (:require [clojure.test :refer [deftest is assert-expr do-report]]
            [clojure.data :refer [diff]]
            [clojure.pprint :refer [pprint]]
            (cosheet2 [utils :refer [multiset-diff]]
                      [test-utils :refer [check]]
                      [canonical :refer [canonicalize-list]])
            (cosheet2.server [hierarchy :refer :all :as hierarchy])
             ; :reload
            ))
...
(deftest append-to-hierarchy-test
  ...
  (is (check (append-to-hierarchy [{::hierarchy/hierarchy-node true
                                    :leaves [:i]
                                    :properties {:a 1}
                                    :cumulative-properties {:a 1}}]
                                  :j {:a 1} {})
  ...
  (is (check
       (append-to-hierarchy [{::hierarchy/hierarchy-node true
                              :properties {:a 1 :b 1}
              ...
              {::hierarchy/hierarchy-node true
               ...
               :child-nodes [{::hierarchy/hierarchy-node true
                             ...
                             {::hierarchy/hierarchy-node true
                              ...
                              :leaves [:l :m ]}]})
             [:i
              {::hierarchy/hierarchy-node true :properties {:c 1} :leaves [:j :k]}
              :l :m]))
