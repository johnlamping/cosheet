...
(defn hierarchy-node-extent
  "Return a seq of descendants of the node that is just big enough
  that their leaves 'cover' all the descendants. That is, for any
  descendant of the node, there is a leaf of the extent whose
  properties are a subset of the properties of the descendant."
  [node]
  (if (seq (:leaves node))
    [(first (:leaves node))]
...
(defn hierarchy-nodes-extent
  "Return a seq of descendants of the nodes that is just big enough
  that their leaves 'cover' all the descendants. That is, for any
  descendant of the node, there is a leaf of the extent whose
  properties are a subset of the properties of the descendant."
  [nodes]
...
;;; The following code assumes that the leaves of a hierarchy are
;;; entity info maps, containing at least the following:
...
(defn item-map-by-elements
  ...
  {:item item
   ...
   :property-canonicals (map entity->canonical-semantic elements)})
