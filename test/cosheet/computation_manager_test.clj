(ns cosheet.computation-manager-test
  (:require [clojure.test :refer [deftest is]]
            [clojure.data :refer [diff]]
            [clojure.pprint :refer [pprint]]
            (cosheet [mutable-map :as mm]
                     [task-queue :as task-queue]
                     [reporters :as reporters :refer [expr cache]]
                     [utils :refer :all]
                     [computation-manager :refer :all])
            ; :reload
            ))

;;; NOTE: The next three functions are not used, but have been
;;; useful in avoiding huge print-outs in debugging.

(defn pretty-expression
  [expr show-prev]
  (if (reporters/reporter? expr)
    (let [desc
          (for [key [:name :value]]
            [key (key (reporters/data expr))])]
      (if (nil? (second (first desc)))
        (let [[key callback] (first (:attendees (reporters/data expr)))]
          (let [to (if (sequential? key) (second key) key)]
            (if (reporters/reporter? to)
              (concat [[:name [(:name (reporters/data to))]]]
                      (rest desc))
              desc)))
        desc))
    (if (sequential? expr)
      (for [e expr]
        (pretty-expression e show-prev))
      expr)))

(defn print-reporter
  [r]
  (doseq [key [:name :manager-type :value]]
    (let [v (key (reporters/data r))]
      (when v
        (println " " key v))))
  (let [source (:value-source (reporters/data r))]
    (when source
      (println " " :value-source (:name (reporters/data source)))))
  (doseq [[rep value] (:subordinate-values (reporters/data r))]
    (println "    Stored" value
             "for" (pretty-expression rep false) (reporters/value rep)))
  (doseq [key (keys (:attendees (reporters/data r)))]
    (println "    callback" (pretty-expression key true))))

(defn print-sources
  [r]
  (print-reporter r)
  (let [source (:value-source (reporters/data r))]
    (when source
      (print "It's source:")
      (print-sources source))))

;;; This function checks that exactly the right callbacks are in
;;; place, and that all copied information is up to date.
(def check-propagation)

(defn check-source-propagation
  [already-checked reporter]
  (let [data (reporters/data reporter)
        source (:value-source data)]
    (if source
      (do (is (= (:value data) (reporters/value source)))
          (is (contains? (:attendees (reporters/data source))
                         `(:copy-value ~reporter)))
          (check-propagation already-checked source))
      already-checked)))

(defn check-subordinate-propagation
  [already-checked reporter]
  (let [data (reporters/data reporter)]
    (reduce
     (fn [checked [subordinate value]]
       (if (contains? (:needed-values data) subordinate)
         checked
         (do
           (is (= value (reporters/value subordinate)))
           (is (contains? (:attendees (reporters/data subordinate)) reporter))
           (check-propagation checked subordinate))))
     already-checked
     (:subordinate-values data))))

(defn check-needed-propagation
  [already-checked reporter]
  (let [data (reporters/data reporter)]
    (reduce
     (fn [checked needed]
       (is (not (reporters/valid? (reporters/value needed))))
       (is (contains? (:attendees (reporters/data needed)) reporter))
       (check-propagation checked needed))
     already-checked
     (:needed-values data))))

(defn check-attendees
  [already-checked reporter]
  (reduce
   (fn [checked [key _]]
     (cond
       (reporters/reporter? key)
       (do (is (or
                (contains? (:subordinate-values (reporters/data key)) reporter)
                (contains? (:needed-values (reporters/data key)) reporter)))
           (check-propagation checked key))
       (list? key)
       (let [user (second key)]
         (is (= (first key) :copy-value))
         (is (reporters/reporter? user))
         (is (= reporter (:value-source (reporters/data user))))
         (check-propagation checked user))
       :else
       checked))
   already-checked
   (:attendees (reporters/data reporter))))

(defn check-propagation
  "Check that all the right information has been propagated to any reporter
  reachable from this one.
  Returns a set of reporters that have already been checked."
  [already-checked reporter]
  (if (contains? already-checked reporter)
    already-checked
    (let [data (reporters/data reporter)]
      (is (= (set (filter reporters/reporter? (:expression data)))
             (clojure.set/union (set (keys (:subordinate-values data)))
                                (:needed-values data))))
      (not (empty? (:attendees data)))
      (-> already-checked
          (conj reporter)
          (check-source-propagation reporter)
          (check-subordinate-propagation reporter)
          (check-needed-propagation reporter)
          (check-attendees reporter)))))

(deftest modify-and-act-test
  (let [r (reporters/new-reporter :test 10)
        a (atom 1)]
    (modify-and-act r (fn [data]
                        (-> data
                            (update-in [:test] inc)
                            (assoc :pending-actions [[swap! a inc]]))))
    (is (= (:test (reporters/data r)) 11))
    (is (= @a 2))))

(deftest copy-value-test
  (let [r1 (reporters/new-reporter :value :v)
        r2 (reporters/new-reporter :value-source r1)]
    (register-copy-value r1 r2)
    (is (= (reporters/value r2) :v))
    (reporters/set-value! r1 :w)
    (is (= (reporters/value r2) :w))
    (swap! (reporters/data-atom r2) dissoc :value-source)
    (register-copy-value r1 r2)
    (reporters/set-value! r1 :x)
    (is (= (reporters/value r2) :w))))

(deftest copy-subordinate-test
  (let [r0 (reporters/new-reporter :name :r0 :value :r0)
        r1 (reporters/new-reporter :name :r1 :value :v)
        r2 (reporters/new-reporter :name :r2
                                  :value :x
                                  :needed-values #{r1}
                                  :subordinate-values {})
        r3 (reporters/new-reporter :name :r3 :value-source r2)
        m (new-management)]
    ;; Register, and check that the information is copied.
    (register-copy-subordinate r1 r2 m)
    (is (= (:needed-values (reporters/data r2)) #{}))
    (is (= (:subordinate-values (reporters/data r2)) {r1 :v}))
    (is (= (reporters/value r2) :x))
    (is (= (task-queue/current-tasks (:queue m))
           [[eval-expression-if-ready r2 m]]))
    ;; Change the subordinate value, and make sure it got propagated.
    (reset! (:queue m) @(task-queue/new-priority-task-queue))
    (reporters/set-value! r1 :v1)
    (is (= (:needed-values (reporters/data r2)) #{}))
    (is (= (:subordinate-values (reporters/data r2)) {r1 :v1}))
    (is (= (reporters/value r2) :x))
    (is (= (task-queue/current-tasks (:queue m))
           [[eval-expression-if-ready r2 m]]))
    ;; Send a spurious update, and make sure things are unchanged.
    (reset! (:queue m) @(task-queue/new-priority-task-queue))
    (reporters/inform-attendees r1)
    (is (= (:needed-values (reporters/data r2)) #{}))
    (is (= (:subordinate-values (reporters/data r2)) {r1 :v1}))
    (is (= (reporters/value r2) :x))
    (is (= (task-queue/current-tasks (:queue m)) ()))
    ;; Now pretend that we did the eval and got a value-source,
    ;; then change the input to undefined, and check all the consequences.
    (reset! (:queue m) @(task-queue/new-priority-task-queue))
    (swap! (reporters/data-atom r2) assoc :value-source r0)
    (register-copy-value r0 r2)
    (register-copy-value r2 r3)
    (is (= (reporters/value r3) :r0))
    (reporters/set-value! r1 reporters/invalid)
    (is (= (reporters/value r2) reporters/invalid))
    (is (= (reporters/value r3) reporters/invalid))
    (is (= (:value-source (reporters/data r2)) r0)) ;; Last know source.
    (is (= (task-queue/current-tasks (:queue m)) ()))
    ;; Now set the value back to the original value, and check the consequences.
    (reporters/set-value! r1 :v1)
    (is (= (:value-source (reporters/data r2)) r0)) ;; Same source.
    (is (= (task-queue/current-tasks (:queue m))
           [[copy-value-callback `(:copy-value ~r2) r0]]))
    (compute m)
    (is (= (reporters/value r2) :r0))
    (is (= (reporters/value r3) :r0))))

(deftest eval-expression-if-ready-test
  (let [r0 (reporters/new-reporter  :name :r0 :value 1)
        r1 (reporters/new-reporter :name :r1
                                  :value 3
                                  :expression [inc 2]
                                  :manager-type :eval)
        r (reporters/new-reporter :name :r
                                 :expression [inc r0]
                                 :needed-values #{r1}
                                 :subordinate-values {r0 1})
        rc (reporters/new-reporter :name :rc :value-source r)
        m (new-management)]
    (register-copy-value r rc)
    ;; Try when the expression is not ready.
    (eval-expression-if-ready r m)
    (is (= (reporters/value r) reporters/invalid))
    (swap! (reporters/data-atom r) assoc :needed-values #{})
    ;; Try when it is ready and computes a constant.
    (eval-expression-if-ready r m)
    (is (= (reporters/value r) 2))
    (is (= (reporters/value rc) 2))
    ;; Try when it is ready and computes a reporter.
    (swap! (reporters/data-atom r)
           #(-> %
                (assoc :subordinate-values {r0 (fn [] r1)})
                (assoc :expression [r0])
                (assoc :value-source r0)))
    (register-copy-value r0 r)
    (is (= (:attendees (reporters/data r0))
           {[:copy-value r] [copy-value-callback]}))
    (is (= (:manager (reporters/data r1)) nil))
    (eval-expression-if-ready r m)
    (is (= (reporters/value r) 3))
    (is (= (reporters/value rc) 3))
    (is (= (:attendees (reporters/data r0)) nil))
    (is (= (:manager (reporters/data r1)) [eval-manager m]))))

(deftest eval-manager-test
  (let [r0 (reporters/new-reporter :value 1)
        r (reporters/new-reporter :expression [inc r0]
                                 :value 2
                                 :manager-type :eval)
        rc (reporters/new-reporter :value-source r)
        m (new-management)]
    ;; Run manager when there is no interest.
    (eval-manager r m)
    (is (= (reporters/value r) reporters/invalid))
    ;; Run when there is interest.
    (register-copy-value r rc)
    (eval-manager r m)
    (is (= (:needed-values (reporters/data r)) #{}))
    (is (= (:subordinate-values (reporters/data r)) {r0 1}))
    ;; Run when there is no interest again.
    (swap! (reporters/data-atom rc) dissoc :value-source)
    (register-copy-value r rc)
    (eval-manager r m)
    (is (not (contains? (reporters/data r) :needed-values)))
    (is (not (contains? (reporters/data r) :subordinate-values)))
    (is (empty? (:attendees (reporters/data r0))))))

(deftest ensure-in-cache-test
  (let [m (new-management)
        r0 (ensure-in-cache [:a :b] "ab" m)
        r1 (ensure-in-cache [:a :c] "bc" m)
        r2 (ensure-in-cache [:a :b] "changed" m)]
    (is (= r0 r2))
    (is (= (:expression (reporters/data r0)) [:a :b]))
    (is (= (:manager-type (reporters/data r0)) :cached-eval))
    (is (not= r0 r1))))

(deftest cache-manager-test
  (let [m (new-management)
        r0 (reporters/new-reporter :name :r0 :value 1)
        r1 (reporters/new-reporter :name :r1
                                  :expression [inc r0]
                                  :manager-type :cache)
        r2 (reporters/new-reporter :name :r2
                                  :expression [inc r0]
                                  :manager-type :cache)
        r3 (reporters/new-reporter :name :r3 :value-source r1)
        r4 (reporters/new-reporter :name :r4 :value-source r2)]
    ;; Nothing should happen when there is no demand for r1.
    (cache-manager r1 m)
    (is (not (contains? (reporters/data r1) :value-source)))
    ;; With demand, the cached value should be created.
    (register-copy-value r1 r3)
    (cache-manager r1 m)
    (is (contains? (reporters/data r1) :value-source))
    ;; And we should pick it up for the other reporter with the same expr.
    (register-copy-value r2 r4)
    (cache-manager r2 m)
    (is (= (:value-source  (reporters/data r1))
           (:value-source  (reporters/data r2))))
    (let [orig-source (:value-source  (reporters/data r1))]
      ;; Lose interest in r1 then get it back, and the same value
      ;; source should come back.
      (swap! (reporters/data-atom r3) dissoc :value-source)
      (register-copy-value r1 r3)
      (cache-manager r1 m)
      (cached-eval-manager orig-source m)
      (is (not (contains? (reporters/data r1) :value-source)))
      (swap! (reporters/data-atom r3) assoc :value-source r1)
      (register-copy-value r1 r3)
      (cache-manager r1 m)
      (is (= (:value-source (reporters/data r1))
             orig-source))
      ;; Now, lose interest in both reporters with that expression,
      ;; and the cache should drop it.
      (swap! (reporters/data-atom r3) dissoc :value-source)
      (register-copy-value r1 r3)
      (cache-manager r1 m)
      (swap! (reporters/data-atom r4) dissoc :value-source)
      (register-copy-value r2 r4)
      (cache-manager r2 m)
      (cached-eval-manager orig-source m)
      (swap! (reporters/data-atom r3) assoc :value-source r1)
      (register-copy-value r1 r3)
      (cache-manager r1 m)
      (is (not= (:value-source (reporters/data r1))
                orig-source)))))

;;; Test that caching is working by doing a recursive computation that would
;;; take a very long time if it weren't cached.
(deftest fib-cache-test
  (let [m (new-management)
        base (reporters/new-reporter :value 0)]
    (letfn [(fib [n] (if (<= n 1)
                       base
                       (expr + (cache fib (- n 1)) (cache fib (- n 2)))))]
      ;; Since the base is 0, fib should be 0 everywhere, and since
      ;; the computations should be cached, this should be fast.
      (let [f45 (fib 45)]
        (is (= (computation-value f45 m) 0))
        (check-propagation #{} f45)
        (reporters/set-value! base 1)
        ;; Now it should be the right value.
        (is (= (computation-value f45 m) 1836311903))
        (check-propagation #{} f45)
        (reporters/set-value! base reporters/invalid)
        ;;; Now it should be invalid.
        (is (= (not (reporters/valid? (computation-value f45 m)))))
        (check-propagation #{} f45)))))


(deftest asynchronous-test
  ;; Creates width base reporters, then a series layers of lookups that
  ;; use the value
  ;; at the previous layer as an index into another value at that
  ;; layer. Makes sure the right results are returned, then starts
  ;; changing the base values in one thread, while another thread
  ;; propagates, and makes sure that the final answer is still right
  (let [width 13
        depth 7
        trials 10; 0000
        changes-per-trial 1000
        base (vec (for [i (range width)]
                    (reporters/new-reporter :name [0 i]
                                           :value (mod (inc i) width))))
        m (new-management)]
    (letfn [(indexer [d pos]
              (if (zero? d)
                (base pos)
                (cache ^{:name [d pos :cache]}
                       indexer-eval d pos)))
            (indexer-eval [d pos]
              (expr ^{:name [d pos :eval]}
                    indexer (- d 1) (indexer (- d 1) pos)))
            (answers [arguments]
               (reduce
                (fn [answers-map [d pos]]
                  (assoc answers-map [d pos]
                         (if (zero? d)
                           (reporters/value (base pos))
                           (answers-map [(- d 1)
                                         (answers-map [(- d 1) pos])]))))
                {}
                arguments))
            (right-results? [requests answers-map]
              (doseq [[[d pos] reporter] requests]
                (is (= (reporters/value reporter)
                       (answers-map [d pos])))))
            (check [requests answers-map]
              (right-results? requests answers-map)
              (reduce check-propagation #{} (vals requests)))]
      (let [arguments (for [d (range depth)
                            pos (range width)]
                        [d pos])
            requests (into {} (for [[d pos] arguments]
                                [[d pos] (request (indexer d pos)m)]))]
        (compute m)
        (check requests (answers arguments))
        (doseq [i (range trials)]
          (when (= (mod i 1000) 0)
            (println "starting trial" i))
          (doseq [j (range changes-per-trial)]
            (reporters/set-value! (base (mod (* i j) width))
                                 (mod (* j j) width))
            (when (zero? (mod j 17))
              (future (compute m))))
          (compute m)
          (check requests  (answers arguments)))))))


