(ns cosheet.cache-calculator-test
  (:require [clojure.test :refer [deftest is]]
            [clojure.pprint :refer [pprint]]
            [clojure.math :as math]
            (cosheet [mutable-map :refer [mm-get current-contents]]
...
(deftest cache-key-test
  (let [r0 (make-reporter :name :r0 :application [:a :b])
    ...
    (is (= (#'cosheet.cache-calculator/cache-key [r1 r0])
           [[:a :b] r0]))
    (is (= (#'cosheet.cache-calculator/cache-key [r2 r0])
           [r2 r0]))
    (is (= (#'cosheet.cache-calculator/cache-key [r1 r11])
           [[:a :b] [[:a :b] [:a :b]]]))))
...
(deftest cache-membership-test
  (let [cd (make-calculator-data (make-priority-task-queue 0))
        data {:application [:a :b]
              :name "ab"
              :cache-key (#'cosheet.cache-calculator/cache-key [:a :b])}
     ...
     (request r0 cd)
     (#'cosheet.cache-calculator/adjust-cache-membership r0 [:a :b] cd)
